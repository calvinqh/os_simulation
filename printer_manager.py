...
class PrinterManager():
    ...
    def snapshot(self):
        for i in range(self.size+1):
            if(i == 0):
                continue
            print('Printer {} Running: {}'.format(i, self.stringify_for_printer_r(i)))
            print('Printer {} Queue: {}'.format(i, self.stringify_for_printer_q(i)))

    def stringify_for_printer_r(self, hd_id):
        t = 'Empty'
        if(self.running_process[hd_id] != None):
            t = self.running_process[hd_id].id
        return t

    def stringify_for_printer_q(self, hd_id):
        temp = []
        result = ''
